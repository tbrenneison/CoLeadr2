...
namespace CoLeadr2.Controllers
{
    public class GroupsController : Controller
    {
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            ...
            //remove all unflagged members from projects
            List<PersonProjectRecord> RemovePprs = new List<PersonProjectRecord>();
            foreach (Project project in group.Projects)
            {
                foreach (PersonProjectRecord ppr in project.PersonProjectRecords)
                {
                    if (ppr.RemoveWithGroup == true)
                    {
                        RemovePprs.Add(ppr);
                    }
                }
            }
            foreach(PersonProjectRecord ppr in RemovePprs)
            {
